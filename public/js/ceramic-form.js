function initCeramicForm(root) {
    ...
    autosuggestInputs.forEach(input => {
        ...
        function loadSuggestions(term = '') {
            ...
            function appendCascadeParams(baseUrl) {
                const currentIndex = cascadeOrder.indexOf(field);
                if (currentIndex <= 0) {
                    return baseUrl;
                }
                ...
                for (let i = 0; i < currentIndex; i++) {
                    const parentField = cascadeOrder[i];
                    if (parentField === 'store' || parentField === 'address' || parentField === field) {
                        continue;
                    }
                    const parentInput = getElement(parentField);
                    const parentValue = (parentInput && parentInput.value ? String(parentInput.value) : '').trim();
                    if (parentValue === '') {
                        continue;
                    }
            ...
            if (field === 'store') {
                ...
                url = `/api/stores/all-stores?search=${encodeURIComponent(term)}&limit=20`;
            } else if (field === 'address') {
                const storeInput = getElement('store');
                if (storeInput && storeInput.value) {
                    url = `/api/stores/addresses-by-store?search=${encodeURIComponent(term)}&limit=20&store=${encodeURIComponent(storeInput.value)}`;
                } else {
                    populate([]);
                    return;
                }

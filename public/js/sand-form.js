function initSandForm(root) {
    ...
    autosuggestInputs.forEach(input => {
        ...
        function appendCascadeParams(url) {
            const cascadeOrder = [
                'type',
                'brand',
            ...
            for (let i = 0; i < currentIndex; i++) {
                ...
                const parentInput = scope.querySelector(`#${parentField}`) || document.getElementById(parentField);
        ...
        function loadSuggestions(term = '') {
            let url = `/api/sands/field-values/${field}?search=${encodeURIComponent(term)}&limit=20`;

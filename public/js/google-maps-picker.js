(function () {
    ...
    function initAddressPicker(options) {
        ...
        const autoFillAddressInput = options?.autoFillAddressInput !== false;
        const syncAddressInputToSearch = options?.syncAddressInputToSearch === true;
        const geocodeAddressInputOnChange = options?.geocodeAddressInputOnChange === true;
            ...
            .then(() => {
                ...
                const setSearchValue = (value) => {
                    ...
                    if (placeAutocompleteElement) {
                        try {
                            placeAutocompleteElement.value = next;
                        } catch (_) {
                ...
                const applyLocation = (place) => {
                    ...
                    if (autoFillAddressInput && streetAddress) {
                ...
                if (syncAddressInputToSearch && addressInput) {
                    const syncFromManualAddress = () => {
                        const manualAddress = String(addressInput.value || '').trim();
                        setSearchValue(manualAddress);
                    };

                    addressInput.addEventListener('input', () => {
                        syncFromManualAddress();
                    });

                    if (geocodeAddressInputOnChange) {
                        const geocodeManualAddress = () => {
                            ...
                            syncFromManualAddress();
                            geocodeAddressText(manualAddress);
                        };

                        addressInput.addEventListener('change', geocodeManualAddress);
                        addressInput.addEventListener('blur', geocodeManualAddress);
                ...
                if (!searchInput.value) {
                    setSearchValue(formattedAddressInput?.value || addressInput?.value || '');
                }

...
function truncateNumber(value, decimals = 2) {
    ...
    function setupWorkTypeAutocomplete() {
        ...
        function ensureFloatingList() {
            if (!(displayInput instanceof HTMLElement) || !(listEl instanceof HTMLElement) || !(document.body instanceof HTMLElement)) {
                return {
                    syncPosition() {},
                    close() {},
                };
            ...
            listEl.dataset.floatingPortal = '1';
            listEl.__autocompleteAnchorEl = displayInput;

            function syncPosition() {
                if (listEl.style.display === 'none' || !displayInput.isConnected) {
                    return;
                }
                ...
                const shouldOpenUp = spaceBelow < preferredHeight && spaceAbove > 120;
                const availableHeight = shouldOpenUp ? spaceAbove : spaceBelow;
                const maxHeight = availableHeight > minHeight ? Math.floor(availableHeight) : defaultMaxHeight;
                const desiredWidth = Math.max(rect.width || 0, 320);
                ...
                const left = Math.max(safeMargin, Math.min(rect.left, maxLeft));

                listEl.classList.toggle('autocomplete-list--up', shouldOpenUp);
                listEl.style.setProperty('position', 'fixed', 'important');
                listEl.style.setProperty('left', Math.round(left) + 'px', 'important');
                listEl.style.setProperty('width', Math.round(width) + 'px', 'important');
                listEl.style.setProperty('min-width', Math.round(width) + 'px', 'important');
                listEl.style.setProperty('max-width', Math.round(maxWidth) + 'px', 'important');
                listEl.style.setProperty('max-height', Math.max(minHeight, maxHeight) + 'px', 'important');
                listEl.style.setProperty('z-index', '20000', 'important');

                if (shouldOpenUp) {
                    listEl.style.setProperty('top', 'auto', 'important');
                    listEl.style.setProperty('bottom', Math.max(safeMargin, Math.round(viewportHeight - rect.top + gap)) + 'px', 'important');
                    return;
                }

                listEl.style.setProperty('bottom', 'auto', 'important');
                listEl.style.setProperty('top', Math.max(safeMargin, Math.round(rect.bottom + gap)) + 'px', 'important');
            }
            ...
            return { syncPosition, close };
        }

        const floatingList = ensureFloatingList();
        ...
        function renderList(items) {
            ...
            if (items.length > 0) {
                floatingList.syncPosition();
                requestAnimationFrame(floatingList.syncPosition);
            }
        }
        ...
        const materialConfigs = {
            ...
            const setupCustomizeAutocomplete = selectEl => {
                ...
                const mirrorCustomizeValueAcrossBundle = nextValue => {
                    ...
                    if (hasAnyChanged && typeof window.__queueCalculationSessionMutation === 'function') {
                        window.__queueCalculationSessionMutation(true);
                    }
                };
                ...
                const applyValue = value => {
                    ...
                    if (typeof window.__queueCalculationSessionMutation === 'function') {
                        window.__queueCalculationSessionMutation(true);
                    }
                    closeList();
                };
                ...
                selectEl.addEventListener('change', function(event) {
                    syncDisplayFromSelect();
                    if (!!event?.isTrusted && typeof window.__queueCalculationSessionMutation === 'function') {
                        window.__queueCalculationSessionMutation(!!event?.isTrusted);
                    }
                });

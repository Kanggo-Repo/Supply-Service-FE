    ...
    <!-- Location Info Card -->
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div class="d-flex align-items-center gap-3">   
                    <a href="{{ route('stores.show', $store) }}" class="btn-cancel" style="background-color: transparent; color: #64748b; padding: 8px 16px; font-size: 14px; font-weight: 600; border-radius: 10px; text-decoration: none; 
                    ...
                    @foreach($materials as $material)
                        <div class="material-tab-action {{ $material['type'] === $activeTab ? 'active' : '' }}" data-tab="{{ $material['type'] }}">
                            <form action="{{ url()->current() }}" method="GET" class="material-search-form manual-search" data-search-manual="true">
                                ...
                                @if(request('search'))
                                    <a href="{{ url()->current() }}?tab={{ $material['type'] }}" class="btn btn-secondary material-search-reset-btn">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadMaterialFormScript(materialType, modalBodyEl) {
        ...
        if (!window[scriptProperty]) {
            ...
            script.src = `/js/${scriptType}-form.js`;
            script.onload = () => {

...
<div class="card store-location-modal">
    ...
    <style>
        ...
        .store-location-modal .store-location-store-banner {
            max-width: 1100px;
            width: 100%;
            margin: 0 auto;
            padding: 0 20px;
        ...
        .store-location-modal .store-location-store-name {
            font-size: 28px;
            line-height: 1.1;
            font-weight: 800;
            color: #0f172a;
    ...
    @php
        $postRedirectUrl = old('_redirect_url', request()->input('_redirect_url'));
        $postRedirectToMaterials = old('_redirect_to_materials', request()->input('_redirect_to_materials'));
        $updateRouteParams = [$store, $location];
        if (filled($postRedirectUrl)) {
            $updateRouteParams['_redirect_url'] = $postRedirectUrl;
        }
        if (filled($postRedirectToMaterials)) {
            $updateRouteParams['_redirect_to_materials'] = $postRedirectToMaterials;
        }
    @endphp

    <form action="{{ route('store-locations.update', $updateRouteParams) }}" method="POST" class="store-location-form">
        @csrf
        @method('PUT')
        @if(filled($postRedirectUrl))
            <input type="hidden" name="_redirect_url" value="{{ $postRedirectUrl }}">
        @endif
        @if(filled($postRedirectToMaterials))
            <input type="hidden" name="_redirect_to_materials" value="{{ $postRedirectToMaterials }}">
        @endif
        ...
        <div class="store-location-store-banner">
            <div class="store-location-store-name">{{ $store->name }}</div>
        </div>

        <div class="form-container store-location-form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; max-width: 1100px; width: 100%; margin: 0 auto; padding: 10px 20px 20px 20px;">
